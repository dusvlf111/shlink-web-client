...
export const UtmTemplateManager: FC = () => {
  ...
  const templateTags = (template: Record<UtmCategory, string>) => UTM_CATEGORIES
    .flatMap((category) => {
            ...
            <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
              {UTM_CATEGORIES.map((cat) => (
                <div key={cat} className="relative">
                  <label htmlFor={`template-${cat}`} className="mb-1 block text-xs font-medium text-gray-700 dark:text-gray-300">
                    utm_{cat}
                  </label>
                  <input
                    id={`template-${cat}`}
                    type="text"
                    value={fields[cat]}
                    onChange={(e) => handleFieldChange(cat, e.target.value)}
                    onFocus={() => setOpenSuggestionField(cat)}
                    onBlur={() => setTimeout(() => setOpenSuggestionField((prev) => (prev === cat ? null : prev)), 120)}
                    autoComplete="off"
                    placeholder={`utm_${cat} 값`}
                  ...
                  />
                  {openSuggestionField === cat && filteredTagValuesByCategory(cat).length > 0 && (
                    <div className="absolute z-10 mt-1 w-full rounded border border-gray-200 bg-white shadow-lg dark:border-gray-700 dark:bg-gray-800">
                      {filteredTagValuesByCategory(cat).map((option) => (
                        <button
                          key={`${cat}-${option.value}`}
                          type="button"
                          onMouseDown={() => handleFieldChange(cat, option.value)}
