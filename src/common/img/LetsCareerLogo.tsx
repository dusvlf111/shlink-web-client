export interface LetsCareerLogoProps {
  className?: string;
}

/**
 * 렛츠커리어 brand mark (the "lets" flame/leaf symbol). The SVG keeps its own
 * brand color (#4D55F5) and is intentionally NOT recolored, so it stays
 * on-brand wherever it is used. On the dark header it is wrapped in a white
 * rounded chip by the caller to guarantee contrast; on light surfaces it can be
 * used bare.
 *
 * Source: lets-intern-client/apps/web/public/logo/logo-simple.svg
 */
export const LetsCareerLogo = ({ className }: LetsCareerLogoProps) => (
  <svg
    className={className}
    width="34"
    height="34"
