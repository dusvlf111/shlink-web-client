...
상세 규칙은 `CLAUDE.md` 및 `folder-structure` 스킬 참조. 요약:
...
├── entities/    # 비즈니스 엔티티
└── shared/      # 완전 재사용 코드 (ui / lib / api / config / i18n / assets)
```
...
- slice 외부에서는 slice의 `index.ts`(public API)만 import
...
├── lib/      # slice 전용 유틸
├── config/   # slice 전용 상수
├── i18n/     # slice 전용 번역 리소스 (ko.json, en.json)
└── index.ts  # public API
```

### i18n — 다국어 지원 (필수)
- **UI에 노출되는 모든 문자열은 `t()`로 분리**. 컴포넌트 내 한국어/영어 하드코딩 금지 (placeholder, aria-label, title 포함)
- slice 전용 텍스트는 `<slice>/i18n/{lang}.json`에 콜로케이션
- 공용 텍스트는 `shared/i18n/locales/{lang}/`로 승격
- 문자열 연결 대신 interpolation(`{{var}}`) 사용
- 네임스페이스 = slice 이름

### 컨벤션
- **스타일**: Tailwind CSS v4 + twMerge
- **상태 관리**: React Hook Form (폼), Zustand (전역), React Query (서버 상태)
- **컴포넌트**: memo 활용, 반응형 분리 (필요시)
- **타입**: 모든 파일에 명시적 타입 정의
- **imports**: path alias `@/` 사용, 상대 경로 `../../` 금지
