...
- [x] 3.0 헤더 레이아웃 및 주요 메뉴 한글화 보완
  - [x] 3.1 헤더 서버 텍스트 세로 배치 버그 수정 (커밋 단위)
    **작업 상세:**
    ...
    **참조:** 문서 src/common/MainHeader.tsx, src/servers/ServersDropdown.tsx
    - [x] 3.1.T1 헤더/서버 드롭다운 레이아웃 테스트 작성 또는 수정
    - [x] 3.1.T2 테스트 실행 및 검증

  - [x] 3.2 미완 한글화 키 보완 (커밋 단위)
    **작업 상세:**
    ...
    **참조:** 문서 src/i18n/locales/ko.ts, src/i18n/locales/en.ts, src/common/UnifiedSidebar.tsx
    - [x] 3.2.T1 i18n 키 누락/불일치 검증 테스트 작성
    - [x] 3.2.T2 테스트 실행 및 검증

  - [x] 3.3 회귀 검증 및 마무리 (커밋 단위)
    **작업 상세:**
    - 헤더, 사이드바, 관련 페이지에서 ko 언어 표시 점검
    - 변경 영향 범위 문서화 및 남은 이슈 목록 정리
    **참조:** 문서 src/common/MainHeader.tsx, src/common/UnifiedSidebar.tsx
    - [x] 3.3.T1 시각 회귀 테스트 또는 스냅샷 업데이트
    - [x] 3.3.T2 npx tsc --noEmit + npm run build 실행 및 검증
